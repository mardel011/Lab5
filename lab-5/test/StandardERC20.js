...
contract("StandardERC20", accounts => {
  ...
  it("test increaseAllowance()", async () => {
    const oldSpenderAllowance = await standardERC20Instance.allowance.call(
      creator,
      spender
    );
    ...
    const increaseSpenderAllowance = new BigNumber(
      oldSpenderAllowance + spenderAmount
    );
    ...
    assert(
      new BigNumber(spenderAmount).isEqualTo(
        new BigNumber(increaseSpenderAllowance)
      ),
      "The increase allowance is not as expected"
    );
  });
  it("test decreaseAllowance()", async () => {
    ...
    const decreaseSpenderAllowance = new BigNumber(
      oldSpenderAllowance - spenderAmount
    );
    ...
    assert(
      new BigNumber(spenderAmount).isEqualTo(
        new BigNumber(decreaseSpenderAllowance)
      ),
      "The decrease allowance is not as expected"
    );
  });
});
